...
def create_visualization(df, chart_type, user_query):
    """Create visualization based on dataframe and chart type with blue color schemes"""
    try:
        if df.empty:
        ...
        if chart_type == "line":
            if datetime_cols and numeric_cols:
                ...
                # Apply blue color
                fig.update_traces(line_color=BLUE_COLORS['discrete'][0], line_width=3)
            ...
            if len(df.columns) >= 2:
                ...
                fig = px.pie(df, names=labels_col, values=values_col, 
                           ...
                           color_discrete_sequence=BLUE_COLORS['pie'])
            ...
            if len(df.columns) >= 2:
                ...
                fig = px.bar(df, x=x_col, y=y_col, 
                           ...
                           color_discrete_sequence=BLUE_COLORS['discrete'])
            ...
            if len(numeric_cols) >= 2:
                ...
                fig = px.scatter(df, x=x_col, y=y_col, 
                               ...
                               color_discrete_sequence=BLUE_COLORS['discrete'])
                # Make scatter points larger and more visible
        ...
        if fig:
            # Apply consistent styling with blue theme
            fig.update_layout(
                showlegend=True,
                height=500,
                ...
                title_font_color=BLUE_COLORS['discrete'][1],
                title_font_size=18,
                font=dict(color=BLUE_COLORS['discrete'][2])
            )
            
            # Update axes colors for better blue theme
            fig.update_xaxes(gridcolor='lightblue', linecolor=BLUE_COLORS['discrete'][3])
            fig.update_yaxes(gridcolor='lightblue', linecolor=BLUE_COLORS['discrete'][3])
            
            # For bar charts, add gradient effect
            if chart_type == "bar":
                fig.update_traces(
                    marker_line_color='white',
                    marker_line_width=1,
...
def get_enhanced_response(user_query: str, db: SQLDatabase, chat_history: list):
    """Enhanced response function with visualization capabilities"""
    try:
        ...
        if needs_viz and chart_created:
            visualization_note = "Note: I've created the requested visualization above showing the data in a beautiful blue-themed chart format."
...
if "chat_history" not in st.session_state:
    st.session_state.chat_history = [
        AIMessage(content="Hello! I'm Analyzee Chatbot. Ask me anything about your database and I'll create beautiful blue-themed visualizations for you!💙"),
    ]

load_dotenv()

st.set_page_config(page_title="Enhanced Analyzee Chatbot", page_icon=":bar_chart:")

st.title("Analyzee Chatbot")

with st.sidebar:
    st.subheader("⚙️ Database Settings")
    st.write("Connect to your MySQL database to start chatting and creating beautiful blue visualizations.")
    
    st.text_input("Host", value="localhost", key="Host")
    st.text_input("Port", value="3306", key="Port")
    st.text_input("User", value="root", key="User")
    st.text_input("Password", type="password", value="chama:1234", key="Password")
    st.text_input("Database", value="database_1", key="Database")
...
user_query = st.chat_input("💬 Ask me about your data or request a blue-themed visualization...")
if user_query is not None and user_query.strip() != "":
    ...
    with st.chat_message("assistant", avatar="🤖"):
        if "db" in st.session_state:
            with st.spinner("🔄 Processing your request and preparing blue visualizations..."):
