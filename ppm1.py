...
def get_pulse_sql_chain(db):
    """Specialized SQL chain for pulse per minute queries using Groq"""
    template = """
    You are a pulse calculation specialist. Based on the table schema below, write SQL queries specifically for pulse per minute calculations.
...
def create_enhanced_visualization(df, chart_type, user_query, detected_metric, message_id=None):
    ...
    try:
        ...
        # Clean and prepare data for other metrics
        df = df.dropna()
        if df.empty:
