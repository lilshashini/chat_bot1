...
def get_enhanced_sql_chain(db):
    ...
    6. For multi-machine production queries by time period:
    ...
    7. For April 2024 data specifically:
    ...
    11. For pulse per minute calculations:
    ...
    uestion: "Show all three machines production by each machine in April with bar chart for each 30 day"
    SQL Query: SELECT 
...
def create_enhanced_visualization(df, chart_type, user_query, detected_metric, message_id=None):
    ...
    try:
        ...
        if chart_type == "multi_machine_bar" or chart_type == "grouped_bar":
            ...
            for col in df.columns:
                if any(keyword in col.lower() for keyword in ['machine', 'device', 'equipment','device_iddevice']):
                    machine_col = col
                    break
